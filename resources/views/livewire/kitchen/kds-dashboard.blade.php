<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($orders->isEmpty())
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-slate-100">No active orders</h3>
                <p class="text-gray-500 dark:text-slate-400 mt-1">Waiting for new orders to arrive...</p>
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($orders as $order)
                    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 overflow-hidden flex flex-col relative"
                        ...
                        <div class="p-4 border-b border-gray-50 bg-gray-50 dark:bg-slate-800/50">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <span class="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Order #{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}</span>
                                    <h3 class="text-lg font-bold text-gray-900 dark:text-slate-100 leading-tight mt-0.5">
                            ...
                            @if($order->customer_name)
                            <div class="text-sm text-gray-600 dark:text-slate-400 flex items-center gap-1.5 mt-2">
                                ...
                                @foreach($order->items as $item)
                                    <li class="flex items-start gap-3">
                                        <div class="flex-shrink-0 mt-0.5">
                                            <span class="inline-flex items-center justify-center h-6 w-6 rounded-md bg-gray-100 dark:bg-slate-700 text-sm font-bold text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-slate-700">
                                                {{ $item->quantity }}
                                            </span>
                                        </div>
                                        <div class="flex-1">
                                            <p class="text-sm font-medium text-gray-900 dark:text-slate-100">{{ $item->menu->name ?? 'Unknown Menu' }}</p>
                                            @if($item->notes)
